        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                ...
                @if($project->has_finished)
                    @if($projectApplicant->status == 'Diterima')
                        <span class="text-white font-bold bg-green-500 rounded-2xl w-full p-5">
                            Yeay, tugas sudah dikonfirmasi selesai dan fee sudah ditambahkan ke saldo kamu! :D
                        </span>
                    @else
                        <span class="text-white font-bold bg-red-500 rounded-2xl w-full p-5">
                            Tugas ini sudah selesai, silahkan melihat tugas tersedia lainnya ya :)
                        </span>
                    @endif
                @else
                    <span class="text-white font-bold bg-yellow-500 rounded-2xl w-full p-5">
                        Tugas masih dalam tahap pengerjaan
                    </span>
                @endif

                <h3 class="text-indigo-950 text-xl font-bold">Kamu telah mengajukan joki pada tugas ini</h3>
                ...
                <div class="item-card flex flex-row gap-y-10 justify-between md:items-center">
                    ...
                    <a href="{{route('front.details', $project->slug)}}" class="font-bold py-4 px-6 bg-blue-500 text-white rounded-full">
                        Lihat Detail
                    </a>
                ...
                <div class="flex flex-row justify-between">
                    <div class="flex flex-row justify-between items-center w-full">
                        <div class="flex flex-row gap-x-4 items-center">
                            <img src="{{Storage::url($projectApplicant->penjoki->avatar)}}" alt="" class="rounded-full object-cover w-[70px] h-[70px]">
                            <div class="flex flex-col">
                                <h3 class="text-indigo-950 text-xl font-bold">
                                    {{$projectApplicant->penjoki->name}}
                                </h3>
                                <p class="text-slate-500 text-sm">
                                    {{$projectApplicant->penjoki->occupation}}
                                </p>
                            </div>
                        </div>
                ...
                @if($projectApplicant->status == 'Diterima')
                    ...
                    <div class="flex flex-row gap-x-3 items-center">
                        ...
                        <div>
                            <h3 class="text-indigo-950 text-xl font-bold">Kamu diterima untuk mengerjakan tugas ini!</h3>
                        <p class="text-slate-500 text-md">
                            Klien akan segera menghubungi email kamu untuk membahas tugas lebih lanjut
                        </p>
                        </div>
                    </div>
                    <hr class="my-5">
                    <h3 class="text-indigo-950 text-xl font-bold">Hubungi Klien Terlebih Dahulu</h3>
                    ...
                    <div class="flex flex-row gap-x-3 items-center">
                        ...
                        <div>
                            <h3 class="text-indigo-950 text-xl font-bold">
                                Fee Rp {{number_format($project->budget, 0, ',', '.')}}
                            </h3>
                        <p class="text-slate-500 text-md">
                            Fee akan ditambahkan ke saldo kamu apabila tugas sudah selesai dan disetujui klien
                        </p>
                        </div>
                    </div>
                @elseif($projectApplicant->status == 'Menunggu')
                @elseif($projectApplicant->status == 'Ditolak')
                    ...
                    <div class="flex flex-row gap-x-3 items-center">
                        ...
                        <div>
                            <h3 class="text-indigo-950 text-xl font-bold">Yah, kamu ditolak oleh klien :(</h3>
                        <p class="text-slate-500 text-md">
                            Silahkan menelusuri tugas lain yang masih tersedia ya
                        </p>
                        </div>
                    </div>
                @endif  
